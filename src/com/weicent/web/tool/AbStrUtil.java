...
public class AbStrUtil {
	 ...
	 *
	 * @param str 指定的车牌号字符串
	 * @return 是否为车牌号格式:是为true，否则false
	 */
	public static Boolean isPlateNo(String str) {
		Boolean isPlateNo = false;
		try {
			Pattern p = Pattern.compile("^(京|津|冀|晋|蒙|辽|吉|黑|沪|苏|浙|皖|闽|赣|鲁|豫|鄂|湘|粤|桂|琼|渝|川|贵|云|藏|陕|甘|青|宁|新|港|澳|台|军|北|南|广|沈|成|兰|济|空|海){1}[A-Z_a-z]{1}[A-Z_a-z_0-9]{5}$");
			Matcher m = p.matcher(str);
			isPlateNo = m.matches();
		} catch (Exception e) {
			e.printStackTrace();
