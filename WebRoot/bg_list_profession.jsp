...
<META http-equiv="Content-Type" content="text/html; charset=UTF-8">
 ...
 function deleteProfession(id) {
  if(confirm("确定要删除吗？")){
    window.location="ProfessionServlet?id="+id+"&action=delete";
	}
  }
 </script>
<html>
	<head>
		<title>Profession列表</title>
			...
			<tr>
				...
				<table width="80%" border="0" align="center" cellpadding="0"
					...
					<table align="center" width="80%" border="1" cellpadding="1"
						...
						%>
						<%
							ResultSet rs = stmt.executeQuery("SELECT COUNT(*) FROM t_profession");
								if (rs.next()) {
								...
								String sql = "SELECT * FROM t_profession "+" ORDER BY "+"id DESC " + " limit " + pageSize * (pageNow - 1) + "," + pageSize;
								...
								if (rs != null) {//字段显示数最多5个，是因为内容页放不下，其他字段需在详情看
									while (rs.next()) {
										...
										String str2 = MyString.cutString(DepartmentsDao.getDepartments(rs.getString(2)).getDepartmentsName(),10,"...") ;		 
						...
						<tr align="center" bgcolor="#FFFFFF">
							...
							<td>
								<a href="bg_detail_profession.jsp?id=<%=str1%>">查看</a>
								<a href="bg_edit_profession.jsp?id=<%=str1%>">编辑</a>
								<a href="javascript:deleteProfession('<%=str1%>')">删除</a>
							</td>
						</tr>
						<%
							}
						...
						<table width="80%" border="0" align="center" cellpadding="0"
							...
							<tr bgcolor="#FFFFFF">
								...
								<%
									//上一页
									if (pageNow != 1) {
										out.println("<a href=bg_list_profession.jsp?pageNow=" + (pageNow - 1)
												+ ">上一页</a>");
									}
									//显示超链接
									for (int i = 1; i <= pageCount; i++) {
										out.println("<a href=bg_list_profession.jsp?pageNow=" + i + ">[" + i
												+ "]</a>");
									}
									//下一页
								...
								<td width="20%">
									...
									&nbsp;
									<a href="bg_add_profession.jsp">添加专业</a>
								</td>
							</tr>
						</table>
					</table>
