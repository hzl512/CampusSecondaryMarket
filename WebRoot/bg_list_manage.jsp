...
<META http-equiv="Content-Type" content="text/html; charset=UTF-8">
 ...
 function deleteManage(id) {
  if(confirm("确定要删除吗？")){
    window.location="ManageServlet?id="+id+"&action=delete";
	}
  }
 </script>
<html>
	<head>
		<title>Manage列表</title>
			...
			<tr>
				...
				<table width="80%" border="0" align="center" cellpadding="0"
					...
					<table align="center" width="80%" border="1" cellpadding="1"
						...
						%>
						<%
							ResultSet rs = stmt.executeQuery("SELECT COUNT(*) FROM t_manage");
								if (rs.next()) {
								...
								String sql = "SELECT * FROM t_manage " + " limit " + pageSize * (pageNow - 1) + "," + pageSize;
						...
						<tr align="center" bgcolor="#FFFFFF">
							...
							<td>
								<a href="bg_detail_manage.jsp?id=<%=str1%>">查看</a>
								<%-- <a href="bg_edit_manage.jsp?id=<%=str1%>">编辑</a> --%>
								<%-- <a href="javascript:deleteManage('<%=str1%>')">删除</a> --%>
							</td>
						</tr>
						<%
							}
						...
						<table width="80%" border="0" align="center" cellpadding="0"
							...
							<tr bgcolor="#FFFFFF">
								...
								<%
									//上一页
									if (pageNow != 1) {
										out.println("<a href=bg_list_manage.jsp?pageNow=" + (pageNow - 1)
												+ ">上一页</a>");
									}
									//显示超链接
									for (int i = 1; i <= pageCount; i++) {
										out.println("<a href=bg_list_manage.jsp?pageNow=" + i + ">[" + i
												+ "]</a>");
									}
									//下一页
								...
								<td width="20%">
									...
									&nbsp;
									<a href="bg_add_manage.jsp">添加管理员</a>
								</td>
							</tr>
						</table>
					</table>
